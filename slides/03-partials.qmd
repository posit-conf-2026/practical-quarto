...
1. Copy, then complete, this line in `title-block.html`, below the subtitle's
   `$endif$`:
...
masthead: _brand/brs-logo.svg   # turn 1: a name at the top level
brs-release:                    # turn 2: a name with names inside
  period: "Q3 2026"
  region: Oregon
```
...
The assistant is a sanctioned escape hatch here. Prompt is in the README.
...
| Write | Get |
|---|---|
| `$if(x)$` … `$else$` … `$endif$` | conditionals, as in turn 3 |
...
`$sep$` is worth ten seconds: it is the answer to the dangling `&middot;` from
turn 2's optional extra — a separator that knows it is between things.
:::
...
::: {.fragment}
No color. No size. No position. Just a name.
:::
...
::: {.fragment}
You have been leaving these hooks since turn 1 — `report-masthead`,
`report-period`, `report-status`. **Three classes, no CSS.**
:::
